...
def dummy_ratings(user_item_dataset, size, score_lst):
    # create a random number of ratings for function testing of collaborating filtering
...
def get_cf_recommendation(user_item_dataset, ratings, estimator, top_n):
    # Provide a dictionary of movie ratings, type of estimator and number of movies recommended,
    # and after fitting data and predicting, lists of best movie Ids and corresponding ratings are returned
    ...
    predicted_ratings = estimator.predict(not_watched_movies)
    best_movie_ids_loc = predicted_ratings.argsort()[:-top_n-1:-1]
    best_movie_ids = not_watched_movie_ids[best_movie_ids_loc]
...
def display_recommendation(movie_df, best_movie_ids, best_movie_scores=None, set_index=True):
    # Dataframe view for displaying recommended movies
    if set_index:
